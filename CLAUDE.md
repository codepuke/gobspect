...
Read `docs/04-architecture.md` for the full design. In brief:
...
- Gob test fixtures are generated by a `testdata/generate.go` helper that encodes known values and writes `.gob` files (run with `go run testdata/generate.go`).
...
Read `docs/03-wire-format.md` for the full reference. Key points:
...
Read `docs/02-opaque-types.md`. Key points:
...
1. Document the binary format in `docs/02-opaque-types.md`.
